CKEDITOR.dialog.add( 'emailvideo', function ( editor )
{
	...
	function commitSrc( videoNode, extraStyles, videos )
	{
		...
		extraStyles.videoUrl = this.getValue();
	}
	...
	return {
		...
		onShow : function()
		{
			...
			if ( fakeImage && fakeImage.data( 'cke-real-element-type' ) && fakeImage.data( 'cke-real-element-type' ) == 'a' )
			{
				this.fakeImage = fakeImage;

				var videoNode = editor.restoreRealElement( fakeImage ),
					videos = [],
					sourceList = videoNode.getElementsByTag( 'img', '' );
				videoUrl = videoNode.getAttribute('href');
				if (sourceList.count()==0)
					sourceList = videoNode.getElementsByTag( 'img', 'cke' );
		...
		onOk : function()
		{
			...
			if ( !this.fakeImage )
			{
				videoNode = CKEDITOR.dom.element.createFromHtml( '<cke:a></cke:a>', editor.document );
				videoNode.setAttributes(
					{
						target : '_blank',
						plugintype : 'emailvideo'						
					} );
			}
			else
			{
			...
			for(var i=0; i<videos.length; i++)
			{
				...
				if ( !video || !video.src )
					continue;
				innerHtml += '<cke:img src="' + extraStyles.posterImage + '"  width="' + extraStyles.width + '" />';				
			}			
			videoNode.setHtml( innerHtml);
			videoNode.setAttribute('href', extraStyles.videoUrl);
			
			// Refresh the fake image.
			var newFakeImage = editor.createFakeElement( videoNode, 'cke_a', 'a', false );
						...
						children : [
							{
								...
								commit : commitValue,
								setup : loadValue,
								validate : CKEDITOR.dialog.validate.notEmpty(lang.videoRequired ),
								onChange : function()
								{
						...
						children : [
							...
							},
							{
								...
								filebrowser :
								{
									action : 'Browse',
									target: 'info:src0',
									url: editor.config.fileEmbedBrowseUrl || editor.config.filebrowserBrowseUrl
								},
