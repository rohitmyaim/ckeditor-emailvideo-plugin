...
CKEDITOR.plugins.add( 'emailvideo',
{
	...
	{
		var lang = editor.lang.emailvideo;
		...
		CKEDITOR.dialog.add( 'emailvideo', this.path + 'dialogs/video.js' );

		editor.addCommand( 'emailvideo', new CKEDITOR.dialogCommand( 'emailvideo' ) );
		editor.ui.addButton( 'emailvideo',
			{
				label : lang.toolbar,
				command : 'emailvideo',
		...
		editor.on( 'doubleclick', function( evt )
			{
				...
				if ( element.is( 'img' ) && element.data( 'cke-real-element-type' ) == 'a' )
					evt.data.dialog = 'emailvideo';
			});
			...
			editor.contextMenu.addListener( function( element, selection )
				{
					if ( element && element.is( 'img' ) && !element.isReadOnly()
							&& element.data( 'cke-real-element-type' ) == 'a' )
						return { video : CKEDITOR.TRISTATE_OFF };
				});
		}

		// Add special handling for these items
		CKEDITOR.dtd.$empty['cke:img']=1;
		CKEDITOR.dtd.$empty['img']=1;
	...
	afterInit: function( editor )
	{
		...
		dataFilter.addRules(
			...
			elements : {
				...
				{						
						if ( realElement.name == 'a' && realElement.attributes.plugintype == 'emailvideo')
						{
							realElement.name = 'cke:a';
							for( var i=0; i < realElement.children.length; i++)
							{
								if ( realElement.children[ i ].name == 'img' )
									realElement.children[ i ].name = 'cke:img'
							}

							var fakeElement = editor.createFakeParserElement( realElement, 'cke_a', 'a', false ),
...
var en = {
		...
		videoRequired: 'Video or Poster url can not be blank',
		poster: 'Poster image',
...
var es = {
		...
		videoRequired: 'Vídeo o cartel url no puede estar en blanco',
...
// Translations
CKEDITOR.plugins.setLang( 'emailvideo', 'en', en );

CKEDITOR.plugins.setLang( 'emailvideo', 'es', es );
